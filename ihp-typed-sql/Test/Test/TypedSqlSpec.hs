...
-- clause. The local @Mood@ type mirrors the @DefaultParamEncoder@ instances the
...
runtimeEnumModule = Text.unlines
    ...
    , "data Mood = Happy | Sad | Neutral deriving (Eq, Show)"
    , ""
    , "moodToText :: Mood -> Text"
    ...
    , "        nothingNames <- sqlQueryTyped [typedSql| SELECT name FROM typed_sql_test_enum_items WHERE opt_mood IS NOT DISTINCT FROM ${(Nothing :: Maybe Mood)} ORDER BY name |]"
