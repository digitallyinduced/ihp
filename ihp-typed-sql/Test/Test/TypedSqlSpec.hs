...
runtimeEnumModule = Text.unlines
    ...
    , "        nothingNames <- sqlQueryTyped [typedSql| SELECT name FROM typed_sql_test_enum_items WHERE opt_mood IS NOT DISTINCT FROM ${(Nothing :: Maybe TypedSqlTestMood)} ORDER BY name |]"
