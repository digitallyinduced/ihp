...
generateGenericApplication applicationName =
        let
            ...
            viewLayoutHs =
                ...
                <> "|]\n"
                <> "\n"
                <> "stylesheets :: (?requestContext :: RequestContext) => MarkupM ()\n"
                <> "stylesheets = do\n"
                <> "    when (isDevelopment $ getConfig environment) [hsx|\n"
                <> "        <link rel=\"stylesheet\" href=\"/vendor/bootstrap.min.css\"/>\n"
                <> "        <link rel=\"stylesheet\" href=\"/vendor/flatpickr.min.css\"/>\n"
                <> "        <link rel=\"stylesheet\" href=\"/app.css\"/>\n"
                <> "    |]\n"
                <> "    when (isProduction $ getConfig environment) [hsx|\n"
                <> "        <link rel=\"stylesheet\" href=\"/prod.css\"/>\n"
                <> "    |]\n"
                <> "\n"
                <> "scripts :: (?requestContext :: RequestContext) => MarkupM ()\n"
                <> "scripts = do\n"
                <> "    when (isDevelopment $ getConfig environment) [hsx|\n"
                ...
                <> "\n"
                <> "\n"
                <> "metaTags :: (?requestContext :: RequestContext) => MarkupM ()\n"
                <> "metaTags = [hsx|\n"
                <> "    <meta charset=\"utf-8\"/>\n"
