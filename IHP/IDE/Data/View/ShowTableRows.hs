    ...
    |]
        where
            ...
            <div class="custom-menu menu-for-column shadow backdrop-blur" id={contextMenuId}>
                <a href={EditRowAction tableName (unsafeTextToUUID primaryKey)}>Edit Row</a>
                <a href={DeleteEntryAction (unsafeTextToUUID primaryKey) tableName} class="js-delete">Delete Row</a>
                <div></div>
                <a href={NewRowAction tableName}>Add Row</a>
            </div>|]
                where
            ...
            renderField primaryKey DynamicField { .. }
                ...
                | isBoolField fieldName tableCols && not (isNothing fieldValue) = [hsx|<td><span data-fieldname={fieldName}><input type="checkbox" onclick={onClick tableName fieldName (unsafeTextToUUID primaryKey)} checked={sqlValueToText fieldValue == "t"} /></span></td>|]
