...
{-# INLINE currentUser #-}
currentUser :: forall user. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl user, Typeable user, user ~ CurrentUserRecord) => user
currentUser = fromMaybe (redirectToLogin (newSessionUrl (Proxy @user))) currentUserOrNothing

{-# INLINE currentUserOrNothing #-}
currentUserOrNothing :: forall user. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl user, Typeable user, user ~ CurrentUserRecord) => (Maybe user)
...
currentUserId :: forall user userId. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl user, HasField "id" user userId, Typeable user, user ~ CurrentUserRecord) => userId
currentUserId = currentUser @user |> get #id

{-# INLINE ensureIsUser #-}
ensureIsUser :: forall user userId. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl user, HasField "id" user userId, Typeable user, user ~ CurrentUserRecord) => IO ()
ensureIsUser =
...
{-# INLINE currentAdmin #-}
currentAdmin :: forall admin. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl admin, Typeable admin) => admin
currentAdmin = fromMaybe (redirectToLogin (newSessionUrl (Proxy @admin))) currentAdminOrNothing

{-# INLINE currentAdminOrNothing #-}
currentAdminOrNothing :: forall admin. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl admin, Typeable admin) => (Maybe admin)
...
currentAdminId :: forall admin adminId. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl admin, HasField "id" admin adminId, Typeable admin) => adminId
currentAdminId = currentAdmin @admin |> get #id

{-# INLINE ensureIsAdmin #-}
ensureIsAdmin :: forall admin adminId. (?controllerContext :: ControllerContext, ?requestContext :: RequestContext, HasNewSessionUrl admin, Typeable admin) => IO ()
ensureIsAdmin =
...
redirectToLoginWithMessage :: (?requestContext :: RequestContext) => Text -> IO ()
...
redirectToLogin :: (?requestContext :: RequestContext) => Text -> a
