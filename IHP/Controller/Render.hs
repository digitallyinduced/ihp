...
renderPolymorphic :: forall viewContext jsonType htmlType. (?context :: ControllerContext) => (MaybeRender htmlType, MaybeRender jsonType) => PolymorphicRender htmlType jsonType -> IO ()
...
render !view = do
    renderPolymorphic PolymorphicRender
            { html = (renderHtml view) >>= respondHtml
