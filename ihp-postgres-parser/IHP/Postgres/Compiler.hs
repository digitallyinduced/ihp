...
compileStatement CreateTrigger { name, eventWhen, event, tableName, for, whenCondition, functionName, arguments } = "CREATE TRIGGER " <> compileIdentifier name <> " " <> compileTriggerEventWhen eventWhen <> " " <> intercalate " OR " (map compileTriggerEvent event) <> " ON " <> compileIdentifier tableName <> " " <> compileTriggerFor for <> maybe "" (\condition -> " WHEN (" <> compileExpression condition <> ")") whenCondition <> " EXECUTE FUNCTION " <> compileExpression (CallExpression functionName arguments) <> ";"
compileStatement CreateConstraintTrigger { name, eventWhen, event, tableName, referencedTableName, deferrable, deferrableType, for, whenCondition, functionName, arguments } = "CREATE CONSTRAINT TRIGGER " <> compileIdentifier name <> " " <> compileTriggerEventWhen eventWhen <> " " <> intercalate " OR " (map compileTriggerEvent event) <> " ON " <> compileIdentifier tableName <> maybe "" (\referencedTable -> " FROM " <> compileIdentifier referencedTable) referencedTableName <> compileDeferrable deferrable deferrableType <> " " <> compileTriggerFor for <> maybe "" (\condition -> " WHEN (" <> compileExpression condition <> ")") whenCondition <> " EXECUTE FUNCTION " <> compileExpression (CallExpression functionName arguments) <> ";"
