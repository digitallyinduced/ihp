...
startPlainGhci = do
    let process = (Process.proc "ghci" ["-threaded", "-isrc", "-fprint-potential-instances", "-fexternal-interpreter", "-fomit-interface-pragmas", "-j2", "-fomit-interface-pragmas", "-prof", "+RTS", "-A128m"]) { Process.std_in = Process.CreatePipe }
