...
tests = do
    ...
    -- Hasql backend tests
    -- These tests verify that the hasql compilation path produces correct SQL.
    -- Key differences from postgresql-simple:
    --   - Uses $1, $2, etc. placeholders instead of ?
    --   - Uses = ANY($1) for IN clauses instead of IN ?
    --   - Uses <> ALL($1) for NOT IN clauses instead of NOT IN ?
    describe "QueryBuilder (Hasql backend)" do
