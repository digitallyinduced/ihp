...
--   * Extracts text values from the 'ConditionValue' via 'getParamPrinterText'
...
matchesInsertPayload (ColumnCondition col op value applyLeft applyRight) row
    ...
    | otherwise = case op of
        ...
        -- For all other operators (InOp, NotEq, Like, regex, range, etc.),
        -- we can't reliably evaluate → safe fallback to refresh.
        -- Note: InOp encodes the entire list as a single PostgreSQL array
        -- parameter, so getParamPrinterText returns one string like
        -- @{\"abc\",\"def\"}@ rather than individual values. We cannot
        -- reliably decompose this, so we fall back to always refreshing.
        _       -> True

-- | Match a column = value condition against a JSON row.
matchEq :: Text -> ConditionValue -> AesonKeyMap.KeyMap Aeson.Value -> Bool
matchEq col (Param params) row =
    case getParamPrinterText params of
        [filterText] -> jsonValueMatchesText (lookupColumn col row) filterText
        _            -> True  -- multi-value or empty, can't evaluate
matchEq col (Literal text) row =
    jsonValueMatchesText (lookupColumn col row) text

-- | Match a column IS value condition (typically IS NULL).
matchIs :: Text -> ConditionValue -> AesonKeyMap.KeyMap Aeson.Value -> Bool
matchIs col (Literal text) row
    | Text.toLower text == "null" =
        case lookupColumn col row of
            Nothing         -> True   -- column not in payload, safe fallback
            Just Aeson.Null -> True   -- matches IS NULL
            Just _          -> False  -- non-null, doesn't match IS NULL
    | otherwise = True  -- IS NOT NULL or other, safe fallback
matchIs _ (Param _) _ = True  -- unexpected, safe fallback
