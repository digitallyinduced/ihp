...
import IHP.Environment qualified as Environment
...
attachOpenApiRenderExpectation :: Maybe DocumentedRenderExpectation -> Request -> Request
attachOpenApiRenderExpectation Nothing request = request
attachOpenApiRenderExpectation (Just expectation) request =
    request{vault = Vault.insert openApiRenderExpectationKey expectation request.vault}
{-# INLINE attachOpenApiRenderExpectation #-}
...
validateOpenApiRenderedResponse
    ...
    -> Maybe JSON.Value
    -> IO ()
validateOpenApiRenderedResponse response actualJson
    | ?request.frameworkConfig.environment /= Environment.Development = pure ()
    | otherwise =
        case Vault.lookup openApiRenderExpectationKey ?request.vault of
            Nothing -> pure ()
            Just DocumentedRenderExpectation{expectedResponseTypeName, expectedTypedJson, expectedJsonSchemaValidationError} ->
                let renderedResponseTypeName = cs (show (Typeable.typeRep (Proxy @response)))
                 in unless
                        (renderedResponseTypeName == expectedResponseTypeName)
                        (throwOpenApiRenderMismatch ("OpenAPI docs expect response " <> cs expectedResponseTypeName <> ", but render produced " <> cs renderedResponseTypeName))
                        >> case (expectedTypedJson (toDyn response), actualJson) of
                            (Nothing, Nothing) ->
                                pure ()
                            (Nothing, Just _) ->
                                throwOpenApiRenderMismatch ("OpenAPI docs do not expect JSON for response " <> cs expectedResponseTypeName <> ", but render produced JSON")
                            (Just _, Nothing) ->
                                throwOpenApiRenderMismatch ("OpenAPI docs expect JSON for response " <> cs expectedResponseTypeName <> ", but render produced no JSON")
                            (Just expectedJson, Just renderedJson) ->
                                unless
                                    (renderedJson == expectedJson)
                                    (throwOpenApiRenderMismatch ("OpenAPI docs expect the JSON generated by response " <> cs expectedResponseTypeName <> ", but render produced a different JSON value"))
                        >> case expectedJsonSchemaValidationError (toDyn response) of
                            Nothing -> pure ()
                            Just validationError ->
                                throwOpenApiRenderMismatch ("OpenAPI docs schema does not match the JSON generated by response " <> cs expectedResponseTypeName <> ":\n" <> cs validationError)
{-# INLINE validateOpenApiRenderedResponse #-}
...
validateOpenApiRenderedView view actualJson =
    validateOpenApiRenderedResponse view (Just actualJson)
                        ...
                        case applyAction @controller constr (queryString waiRequest) of
                            ...
                            Right action -> do
                                case parseMethod (requestMethod waiRequest) of
                                    ...
                                    Left err -> Exception.throw BadHttpMethodException{method = err}
                 )
