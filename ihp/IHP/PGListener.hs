...
notifyLoop listeningToVar listenToVar subscriptions = do
    ...
    let innerLoop = do
            -- Acquire a dedicated hasql connection for listening
            let dbUrl = ?modelContext.databaseUrl
            connResult <- Hasql.acquire [HasqlSetting.connection (HasqlConnection.string (cs dbUrl))]
            case connResult of
                Left err -> error ("PGListener: Failed to acquire connection: " <> show err)
                Right conn -> do
