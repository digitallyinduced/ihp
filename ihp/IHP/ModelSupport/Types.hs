...
-- | The ordered list of a model's field names, in 'touchedFields' bit order.
-- The schema compiler emits one @type instance ModelFields (Model') = '[...]@
-- per table. The field at index @n@ owns bit @2^n@.
type family ModelFields model :: [Symbol]

-- | 0-based position of @name@ within a field list. Used to derive the
-- 'touchedFields' bit for a field.
type family FieldIndex (name :: Symbol) (fields :: [Symbol]) :: Nat where
    FieldIndex name (name ': rest)  = 0
    FieldIndex name (other ': rest) = 1 + FieldIndex name rest

-- | Maps a field name to its bit position in the 'touchedFields' bitmask.
--
-- Rather than emitting one @instance FieldBit@ per field per table (which adds
-- up to thousands of tiny instances in real apps), the schema compiler emits a
-- single type-level field list per table via 'ModelFields', and 'fieldBit'
-- computes the bit from the field's position in that list.
--
-- 'FieldBit' is a constraint synonym (not a class): for a concrete table it
-- reduces to a 'KnownNat' that GHC solves to a literal, and for code that is
-- polymorphic over the model (e.g. 'didChange', 'selectField') it is carried as
-- an ordinary dictionary — exactly like the old per-field class instances.
type FieldBit (name :: Symbol) model = KnownNat (2 ^ FieldIndex name (ModelFields model))

-- | The 'touchedFields' bit owned by the field @name@ of @model@, i.e.
-- @2 ^ fieldPosition@. Folds to a compile-time literal for concrete models, so
-- this is identical runtime code to the previously generated per-field instances.
fieldBit :: forall name model. FieldBit name model => Integer
fieldBit = natVal (Proxy @(2 ^ FieldIndex name (ModelFields model)))
{-# INLINE fieldBit #-}
