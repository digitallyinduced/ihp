    ...
    ghc = final.haskell.packages.ghc98.override {
        overrides = self: super:
            let
                ...
                localPackage = name: super.callCabal2nix name (filter { root = "${toString flakeRoot}/${name}"; include = [ (filter.matchExt "hs") (filter.matchExt "cabal") (filter.matchExt "md") (filter.matchExt "json") (filter.matchExt "sql") filter.isDirectory "LICENSE" "data" ]; }) {};
        in {
