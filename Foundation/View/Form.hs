...
import Foundation.ModelSupport (getModelName, GetModelName)
...
submitButton = let modelName = Foundation.ModelSupport.getModelName @model in SubmitButton
    ...
    , label = cs $ (if Foundation.ModelSupport.isNew (model ?formContext) then "Create " else "Save ") <> modelName
    , buttonClass = mempty
    }
...
instance (
    ...
    fromLabel = \(formContext, _) -> let columnName = (cs $ getField @symbol (Foundation.ModelSupport.columnNames (Proxy @model))) in FormField {
                        ...
                        fieldInputId = cs (Foundation.NameSupport.lcfirst (getModelName @model) <> "_" <> columnName),
