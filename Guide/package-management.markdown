...
## Moving a Local Library to GitHub

Start with `lib/my-client` while the package is specific to one application. Move it into a separate GitHub repository when it is shared by multiple applications, needs its own CI, or should have an independent release lifecycle.

Keep the same package structure in the new repository:
...
The `default.nix` file can stay generated by `cabal2nix`:

```bash
nix run nixpkgs#cabal2nix -- . > default.nix
```

Add a small `flake.nix` to the library repository so it can be checked independently:
...
Then add the GitHub repository as an input to your IHP application's `flake.nix`:
...
inputs = {
    ...
    my-client.url = "github:my-org/my-client";
};
```

Make sure the `outputs` function receives the new input:

```nix
outputs = inputs@{ ihp, flake-parts, systems, my-client, ... }:
```

Finally, add the package to `haskellPackages` using the IHP application's package set:
...
Prefer this `p.callPackage` form over `my-client.packages.${pkgs.system}.default` inside an IHP app. The package is still pinned through the flake input, but it is compiled with the same Haskell package set as the application.
