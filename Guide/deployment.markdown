...
A fresh NixOS image only knows the `root` account, so the first login is a root login. See [Deploying without root](#deploying-without-root) for switching to an unprivileged deploy user afterwards.
...
This will connect to the server via SSH and apply the NixOS configuration to the server. The argument must match both your SSH host name and the `nixosConfigurations` key in your `flake.nix`. An optional `user@` prefix, as in `deploy-to-nixos deploy@production`, changes only the SSH login; the configuration key stays `production`.

Any further arguments are passed on to `nixos-rebuild`, e.g. `deploy-to-nixos production --show-trace`.

### Deploying without root

`deploy-to-nixos` does not need a root login. Whenever the SSH user is not `root`, the tool lets `nixos-rebuild` elevate the activation on the server with `sudo` and starts `migrate.service` through `sudo` as well. The user comes from the `user@` prefix or, when there is none, from your `~/.ssh/config`.

The deploy user needs to be declared in your `configuration.nix`:
...
# A deployment has no terminal to type a sudo password into
...
# Nix ignores the IHP binary cache and `--option sandbox false` for untrusted
# users, and both are passed to the build that runs on the server
...
The SSH host name in `~/.ssh/config` must match the `nixosConfigurations` key in your `flake.nix`. Store `SSH_PRIVATE_KEY` and `SSH_HOST` as GitHub repository secrets. The `deploy` user has to exist on the server already, see [Deploying without root](#deploying-without-root).
                ...
                "${pkgs.writeShellApplication {
                    ...
                    text = ''
                        ...
                        exec ${pkgs.nixos-rebuild}/bin/nixos-rebuild switch \
                            ...
                            --sudo \
                            --target-host deploy@production \
                            --build-host deploy@production
                    '';
                }}/bin/deploy";
        };
    };
...
The `--use-substitutes` flag lets the server pull pre-built packages from binary caches, and `--build-host` builds on the server itself (useful when deploying from a different architecture, e.g. macOS to Linux). `--sudo` elevates the activation on the server, so the login user does not have to be root; older `nixos-rebuild` versions call this flag `--use-remote-sudo`.
