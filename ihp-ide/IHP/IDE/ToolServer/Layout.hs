...
|]  where
        ...
        (AvailableApps appNames) = unsafePerformIO (readIORef (lookupRequestVault availableAppsVaultKey ?context.request))
...
appUrl = let (AppUrl url) = unsafePerformIO (readIORef (lookupRequestVault appUrlVaultKey ?context.request)) in url
