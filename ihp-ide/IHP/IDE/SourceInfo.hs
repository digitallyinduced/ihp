...
getSourceInfo = do
    ...
    -- When set, it points to <ihp-root>/ihp-ide/lib/IHP for local checkouts.
    -- When not set, fall back to checking the executable path.
    ...
    case ihpLib of
        ...
        Nothing -> do
            ...
            if "/nix/store/" `isPrefixOf` (cs exePath :: Text)
