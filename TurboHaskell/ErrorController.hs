...
handlePatternMatchFailure :: (Show controller, ?requestContext :: RequestContext) => Exception.PatternMatchFail -> controller -> _
handlePatternMatchFailure exception controller = do
    ...
    respond $ responseBuilder status500 [(hContentType, "text/html")] (Blaze.renderHtmlBuilder (renderError errorMessage))




renderError :: _
renderError view = H.docTypeHtml ! A.lang "en" $ do
    H.head do
    ...
    H.body ! A.style "margin: 0; font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", \"Roboto\", \"Helvetica Neue\", Arial, sans-serif;" $ do
        H.div ! A.style "background-color: #657b83; padding-top: 2rem; padding-bottom: 2rem; color:hsla(196, 13%, 96%, 1)" $ do
            H.div ! A.style "max-width: 800px; margin-left: auto; margin-right: auto" $ do
                H.h1 ! A.style "margin-bottom: 2rem; font-size: 2rem; font-weight: 300; border-bottom: 1px solid white; padding-bottom: 0.25rem; border-color: hsla(196, 13%, 60%, 1)" $ "Missing action"
