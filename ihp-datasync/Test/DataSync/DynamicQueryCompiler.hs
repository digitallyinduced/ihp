    ...
    describe "IHP.DataSync.DynamicQueryCompiler" do
        describe "compileQuery" do
            ...
            it "compile a basic select query with a where condition" do
                ...
                sqlText `shouldSatisfy` (\t -> "SELECT" `isPrefixOf` t)
                sqlText `shouldSatisfy` (\t -> "\"posts\"" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "WHERE" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "\"user_id\"" `isInfixOf` t)
            ...
            it "compile a basic select query with a limit" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "LIMIT" `isInfixOf` t)
            ...
            it "compile a basic select query with an offset" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "OFFSET" `isInfixOf` t)
            ...
            it "compile 'field = NULL' conditions to 'field IS NULL'" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "IS" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "NULL" `isInfixOf` t)
            ...
            it "compile 'field <> NULL' conditions to 'field IS NOT NULL'" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "IS NOT" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "NULL" `isInfixOf` t)
            ...
            it "compile 'field IN (NULL, 'string')' conditions to 'field IS NULL OR field IN ('string')'" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "OR" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "IS" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "IN" `isInfixOf` t)
            ...
            it "compile queries with TS expressions" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "@@" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "to_tsquery" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "ts_rank" `isInfixOf` t)
            ...
            it "compile a basic select query with distinctOn" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "DISTINCT ON" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "\"group_id\"" `isInfixOf` t)
            ...
            it "compile a WHERE IN query" do
                ...
                let sqlText = snippetToSql (compileQuery query)
                sqlText `shouldSatisfy` (\t -> "IN" `isInfixOf` t)
                sqlText `shouldSatisfy` (\t -> "\"id\"" `isInfixOf` t)
