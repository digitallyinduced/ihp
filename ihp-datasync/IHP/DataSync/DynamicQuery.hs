...
dynamicRowJsonDecoder = do
    ...
    case jsonToFields jsonValue of
        ...
        Left err -> error ("dynamicRowJsonDecoder: Failed to decode JSON row: " <> err)
...
jsonToFields other = Left ("Expected JSON object but got: " <> show other)

-- | Converts an Aeson Value to a DynamicValue
aesonToDynamicValue :: Value -> DynamicValue
aesonToDynamicValue Data.Aeson.Null = IHP.DataSync.DynamicQuery.Null
