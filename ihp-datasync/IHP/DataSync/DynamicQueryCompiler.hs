...
compileQueryMapped DynamicSQLQuery { .. } =
    ...
    where
        ...
        limitSnippet = case limit of
                Just l  -> Snippet.sql " LIMIT " <> Snippet.param l
                Nothing -> mempty

        offsetSnippet = case offset of
                Just o  -> Snippet.sql " OFFSET " <> Snippet.param o
                Nothing -> mempty
...
dynamicValueParam (IntValue int) = Snippet.param int
...
dynamicValueParam (PointValue point) = Snippet.param point
