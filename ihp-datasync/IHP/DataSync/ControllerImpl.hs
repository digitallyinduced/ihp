...
buildMessageHandler hasqlPool ensureRLSEnabled installTableChangeTriggers sendJSON handleCustomMessage renamer columnTypeLookup = do
    ...
    where
            ...
            handleMessage getRLSColumns CreateDataSubscription { query, requestId, clientSubscriptionId } = do
                ...
                -- Keep server-assigned ids for older clients in a separate namespace
                -- from the non-negative ids generated by current clients.
                let subscriptionId = fromMaybe ((-requestId) - 1) clientSubscriptionId
