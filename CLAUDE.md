...
## Job Module Architecture Notes (`ihp/IHP/Job/`)

`IHP.Job` follows an index-module + focused-submodules structure to keep public imports stable while reducing module size.

### Stable public entrypoints (keep compatible)

- `IHP.Job.Types`
- `IHP.Job.Queue`
- `IHP.Job.Runner`

These modules are the API surface for users and should keep explicit export lists and backward-compatible names/signatures.

### Internal organization

- `IHP.Job.Types.*`
  - `BackoffStrategy`: backoff data types
  - `Class`: `Job` typeclass and defaults
  - `Status`: `JobStatus`
  - `Worker`: worker/process types (`JobWorkerArgs`, `JobWorker`, `JobWorkerProcess`, messages)
- `IHP.Job.Queue.*`
  - `Pool`: pooled Hasql execution (`runPool`)
  - `Fetch`: job selection/locking SQL (`fetchNextJob`, pending condition)
  - `Watch`: pg_notify + poller + trigger repair path
  - `Result`: state transitions (`jobDidFail`, `jobDidTimeout`, `jobDidSucceed`, `recoverStaleJobs`)
  - `StatusInstances`: `JobStatus` DB/param encoding-decoding instances and enum mapping
  - `STM`: queue utility helpers (`tryWriteTBQueue`)
- `IHP.Job.Runner.*`
  - `MainLoop`: process lifecycle + signal handling
  - `WorkerLoop`: fetch/execute loop per worker

### Conventions for future edits

- Put shared/user-facing exports in the 3 index modules and keep submodules internal.
- Keep `IHP.Job.Queue` importing/re-exporting status instance visibility behavior via `StatusInstances`.
- Use `IHP.Prelude` directly in job submodules (no `IHP.Job.Prelude` wrapper module).
