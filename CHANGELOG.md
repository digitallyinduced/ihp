...
- Fixed `typedSql`'s automatic compile-time database (`IHP_TYPED_SQL_AUTO_DB`) leaking temporary PostgreSQL clusters. The cleanup monitor only started once the cluster was fully up, so a compiler process killed before that — Ctrl-C, a cancelled parallel build, the OOM killer — left its directory, and sometimes a running postgres, behind forever. Enough of them exhaust SysV shared memory, after which every later `typedSql` compile fails during `initdb` with `could not create shared memory segment: No space left on device`. The monitor now starts before `initdb` and records its owning process, and each run reaps directories whose owner is gone.
