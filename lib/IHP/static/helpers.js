...
function initDelete() {
    document.querySelectorAll('.js-delete').forEach(function (elem) {
        if (elem.hasAttribute('jsDeleteInitialized') === false) {
            elem.addEventListener('click', handleClick);
            elem.setAttribute('jsDeleteInitialized', true);
        }
    });
...
window.transitionToNewPage = function (newHtml) {
    ...
    morphdom(document.body, newHtml.tagName === 'BODY' ? newHtml : newHtml.body, {
        ...
        onBeforeElUpdated: function (from, to) {
            if (newHtml.body && newHtml.body.classList.contains('modal-open') && from.id === 'main-row') {
                return false;
