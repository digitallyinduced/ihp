...
window.submitForm = function (form, possibleClickedButton) {
    ...
    // Let Turbo handle form submissions when available
    if (window.Turbo) {
        ...
        // Disable buttons and show alerts like before
        ...
        // Let Turbo handle the form submission automatically
        if (possibleClickedButton &&
            ((possibleClickedButton instanceof HTMLInputElement && possibleClickedButton.type === 'submit') ||
                (possibleClickedButton instanceof HTMLButtonElement && possibleClickedButton.type === 'submit'))) {
            form.requestSubmit(possibleClickedButton);
        } else {
            form.requestSubmit();
        }
        return;
    ...
    // Fallback for when Turbo is not available - keep existing XMLHttpRequest logic
    ...
    request.onload = function () {
        ...
        // Dispatch turbo:load event for compatibility with Turbo-aware code
        var turboLoadEvent = new CustomEvent('turbo:load');
        document.dispatchEvent(turboLoadEvent);
...
window.transitionToNewPage = function (newHtml) {
    ...
    // Handle auto-refresh meta tag updates
