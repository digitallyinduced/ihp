...
export class LiveQuery {
    constructor(query, variables = null) {
        if (typeof query !== "string") {
            throw new Error("Query passed to `new LiveQuery(..)` doesn't look like a GraphQL query string.`");
        }
        this.query = query;
        ...
        this.newRecordBehaviour = APPEND_NEW_RECORD
    }

    async createOnServer() {
        const dataSyncController = DataSyncController.getInstance();
        try {
            const { liveQueryId, graphQLResult } = await dataSyncController.sendMessage({ tag: 'CreateGraphQLLiveQuery', gql: this.query, variables: this.variables === null ? {} : this.variables });
            ...
            this.isConnected = true;
            this.isClosed = false;
            this.records = graphQLResult;

            this.resolveCreateOnServer(graphQLResult);
            this.updateSubscribers();
        } catch (e) {
            this.connectError = new Error(e.message + ' while trying to subscribe to:\n' + JSON.stringify(this.query, null, 4));
            this.rejectCreateOnServer(this.connectError);
    ...
    onMessage(message) {
        ...
        if (message.liveQueryId === this.liveQueryId) {
            this.receiveUpdate(message);
        }
    }
    ...
    receiveUpdate(message) {
        const tag = message.tag;
        if (tag === 'LiveQueryDidUpdate') {
            this.onUpdate(message.id, message.changeSet);
        } else if (tag === 'LiveQueryDidInsert') {
            this.onCreate(message.newRecord, message.table);
        } else if (tag === 'LiveQueryDidDelete') {
            this.onDelete(message.id, message.table);
        }
    }
    ...
    async close() {
        ...
        const dataSyncController = DataSyncController.getInstance();
        const { liveQueryId } = await dataSyncController.sendMessage({ tag: 'DeleteDataSubscription', subscriptionId: this.liveQueryId });
    ...
    onUpdate(id, changeSet) {
        const applyPatch = (result) => {
            if (result instanceof Array) {
                return result.map(applyPatch);
            }
            if (typeof result === 'object') {
                if ('id' in result && result.id === id) {
                    return Object.assign({}, result, changeSet);
                } else {
                    const newResult = {};
                    for (let key in result) {
                        newResult[key] = applyPatch(result[key]);
                    }
                    return newResult;
                }
            }
        ...
        this.records = applyPatch(this.records);
        console.log('New', this.records);

        this.updateSubscribers();
    }

    onCreate(newRecord, table) {
        const shouldAppend = this.newRecordBehaviour === APPEND_NEW_RECORD;
        // TODO: this is very terrible, we should rewrite this to a path-based approach
        this.records = {
            ...this.records,
            [table]: [...this.records[table], newRecord]
        };
        this.updateSubscribers();
    }
    ...
    onDelete(id, table) {
        this.records = {
            ...this.records,
            [table]: this.records[table].filter(record => record.id !== id)
        };
        this.updateSubscribers();
    }
